		...
		for(let property in object){
			...
			if(Array.isArray(value)){
                ...
                for(let element in array){
                    key += '[' + element + ']'
					hash = Object.assign(hash, this.to_h(array[element], key, hash))
				...
				hash = Object.assign(hash, this.to_h(primitive, primitive_key, hash))
			}
			else if(typeof value === 'object'){
                let obj = value
				let nested_key = (key) ? key + '['+ property + ']' : property
                hash = Object.assign(hash, this.to_h(obj, nested_key, hash))
			}
		}
		return hash
	}
